...
export default defineConfig(({ command, mode }) => {
  ...
  // command: 'serve' (H5 dev) | 'build' (MP/App dev and all production builds)
  // dev 和 build 命令分别使用 .env.development 和 .env.production 的环境变量

  const { UNI_PLATFORM } = process.env // 得到 'h5' | 'mp-weixin' | 'app' 等
  ...
  return defineConfig({
    ...
    plugins: [
      ...
      UniPages({
        ...
        // "src/pages-demo" 为分包示例目录，可按需插入演示页面
