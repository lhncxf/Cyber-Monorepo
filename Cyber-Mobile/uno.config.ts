...
const RE_SVG_FILL = /^<svg /
const RE_SVG_WIDTH = /(<svg.*?width=)"(.*?)"/
const RE_SVG_HEIGHT = /(<svg.*?height=)"(.*?)"/
...
export default defineConfig({
        ...
        'my-icons': FileSystemIconLoader(
          ...
          (svg) => {
            ...
            svgStr = svgStr.includes('fill="')
              ? svgStr
              : svgStr.replace(RE_SVG_FILL, '<svg fill="currentColor" ')

            // 如果 svg 有 width, 和 height 属性，将这些属性改为 1em，否则无法显示图标
            svgStr = svgStr
              .replace(RE_SVG_WIDTH, '$1"1em"')
              .replace(RE_SVG_HEIGHT, '$1"1em"')
  ...
  theme: {
    colors: {
      ...
      border: 'var(--brand-border,#f0ece6)',
    },
    fontSize: {
