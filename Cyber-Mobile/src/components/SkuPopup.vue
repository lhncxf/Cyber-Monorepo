...
<template>
  <view v-if="visible" class="fixed inset-0 z-50" @tap.stop="close">
    ...
    <view
      class="absolute bottom-0 left-0 right-0 rounded-t-2xl bg-white pb-safe"
    ...
    >
      <view class="flex items-start gap-3 p-4">
        ...
        <view class="flex-1 pt-1">
          <view v-if="selectedSku" class="mb-1 text-xl font-bold" :class="isSoldOut ? 'text-gray-400' : 'text-red-500'">
            ¥{{ selectedSku.price.toFixed(2) }}
          </view>
          <view v-else class="mb-1 text-xl text-red-500 font-bold">
            —
          </view>
        ...
        <view class="text-2xl text-gray-300 leading-none" @tap="close">
          ×
        </view>
      </view>
      ...
      <view class="px-4 pb-2">
        ...
        >
          ...
          <view class="flex flex-wrap gap-2">
            ...
            >
              {{ val }}
              <text v-if="isValueSoldOut(dimIdx, val)" class="absolute rounded bg-white px-0.5 text-[10px] text-gray-400 leading-none -right-1 -top-1.5">{{ t('sku.soldOutTag') }}</text>
            </view>
          </view>
        </view>
      </view>
      ...
      <view class="mx-4 mb-4 flex items-center justify-between border-t border-gray-100 pt-4">
        ...
        <view class="flex items-center gap-4">
          <view
            class="h-8 w-8 flex items-center justify-center border border-gray-200 rounded text-gray-600"
            @tap="changeQty(-1)"
          >
            -
          </view>
      ...
      <view class="mx-4 mb-4">
        <button
          ...
          :class="isSoldOut ? 'bg-gray-300' : 'bg-red-500'"
          :disabled="isSoldOut"
          @tap="confirm"
        >
