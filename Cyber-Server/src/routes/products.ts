...
function formatProduct(p: {
  ...
  return {
    ...
    status: p.status,
    createTime: p.createTime.toISOString().replace("T", " ").slice(0, 19),
...
const productInclude = {
  ...
  skus: { orderBy: { id: "asc" as const } }
}

const productIncludeActiveSkus = {
  category: { select: { id: true, name: true } },
  freightTemplate: { select: { id: true, name: true } },
  skus: { where: { status: true }, orderBy: { id: "asc" as const } }
}
  ...
  app.get<{
    ...
    async (request, reply) => {
      ...
      const list = await prisma.product.findMany({
        ...
        orderBy: { id: "desc" },
        include: productIncludeActiveSkus
      })
        ...
        for (const sku of skus) {
          if (sku.id) {
            ...
            await prisma.productSku.create({
              data: { productId: id, name: sku.name, price: sku.price, stock: sku.stock ?? 0, status: sku.status ?? true }
            })
          }
        }
      }
