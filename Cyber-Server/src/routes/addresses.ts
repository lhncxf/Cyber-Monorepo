...
export async function addressesRoutes(app: FastifyInstance) {
  ...
  app.post<{
    ...
    async (request, reply) => {
      ...
      // Atomic default toggle: clear existing default first
      if (isDefault) {
      ...
      const existingCount = await prisma.shippingAddress.count({ where: { userId: mobileUserId } })
      const shouldBeDefault = isDefault || existingCount === 0

      if (shouldBeDefault && existingCount > 0 && !isDefault) {
        // clear existing defaults if setting this as default
        await prisma.shippingAddress.updateMany({
          where: { userId: mobileUserId, isDefault: true },
          data: { isDefault: false }
        })
  ...
  app.delete<{ Params: { id: string } }>(
    ...
    async (request, reply) => {
      ...
      // If the deleted address was default, auto-promote the latest remaining address
  ...
  app.put<{ Params: { id: string }, Body: Record<string, never> }>(
    ...
    async (request, reply) => {
      ...
      await prisma.$transaction([
        ...
        prisma.shippingAddress.update({ where: { id }, data: { isDefault: true } })
      ])
