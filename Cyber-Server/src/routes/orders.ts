...
function formatMobileOrder(order: MobileOrder) {
  return {
    ...order,
...
export async function ordersRoutes(app: FastifyInstance) {
  ...
  app.get<{
    ...
    async (request, reply) => {
      ...
      return reply.send({ code: 0, data: { list: list.map(formatMobileOrder), total }, message: t("querySuccess", lang) })
    }
  )
  ...
  app.get<{ Params: { id: string } }>(
    ...
    async (request, reply) => {
      ...
      return reply.send({ code: 0, data: formatMobileOrder(order), message: t("querySuccess", lang) })
    }
  )
}
